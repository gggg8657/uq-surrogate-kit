...
def sec_h16(sc, out):
    ...
    if acc:
        ...
        out.append(
            ...
            + (f" The `darcy_amp2` control is unchanged (ratio "
               f"{ctlv[0]['ratio']:.4f}, still "
               f"{ctlv[0]['eq_over_in_dist']:.2f}\u00d7 the in-distribution "
               f"error). **Read that control narrowly.** Darcy's `amp` "
               f"multiplier reaches the input through channel 0 "
               f"(log-permeability) while the wrapper rescales channel 1 "
               f"(the forcing), which `amp` never touches \u2014 so the "
               f"scale it measures is ~1 and it is a near-identity *by "
               f"construction*. This shows the wrapper does not fire on a "
               f"shift that moves a channel it does not rescale; it does "
               f"**not** show that the algebra correctly declines to apply to "
               f"a nonlinear channel, because the wrapper never reads that "
               f"channel. An audit (`logs/critic_codex_h17.log`) named this "
               f"and it is correct."
               if ctlv else "")
            + f" Every shard that is *not* an amplitude shift moves by at most "
            f"**{e['max_abs_rel_l2_change_off_amp']:.1e}** in relative error \u2014 "
            f"a bound on the change in each shard's *median* error, not a "
            f"per-sample invariance guarantee.\n")
        out.append(
